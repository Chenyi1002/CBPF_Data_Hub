(function d3ChartIIFE() {
	...
	function d3Chart() {
		...
		function dispatchYearSelectionToPage() {
			const numericYears = chartState.selectedYear
				.filter(function (year) {
					return year !== allYearsOption && Number.isFinite(+year);
				})
				.map(function (year) {
					return +year;
				});

			const selectedYearForComparison =
				numericYears.length === 1 ? numericYears[0] : null;

			const detail = {
				years: chartState.selectedYear.slice(),
				year: selectedYearForComparison,
			};

			try {
				window.dispatchEvent(
					new CustomEvent("pbiclcYearChanged", {
						detail: detail,
					})
				);
			} catch (error) {
				const event = document.createEvent("CustomEvent");
				event.initCustomEvent("pbiclcYearChanged", false, false, detail);
				window.dispatchEvent(event);
			}
		}

		function syncBrowserUrlFromChartQueryString() {
			try {
				window.history.replaceState(
					null,
					"",
					window.location.pathname + "?" + queryStringValues.toString()
				);
			} catch (error) {
				// Keep the chart working even if the embedding page blocks history updates.
			}
		}
		...
		const selectedCountriesString = queryStringValues.has("country")
			? queryStringValues.get("country").replace(/\|/g, ",")
			: containerDiv.node().getAttribute("data-selectedcountry");
		...
		const selectedContribution =
			queryStringValues.has("contribution") &&
		...
		const topDiv = containerDiv.append("div").attr("class", "pbiclcTopDiv");

		const titleDiv = topDiv.append("div").attr("class", "pbiclcTitleDiv");
		...
		const footerDiv = !isPfbiSite
			? containerDiv.append("div").attr("class", "pbiclcFooterDiv")
			: null;
		...
		const donorsPanelClip = donorsPanel.main
			.append("clipPath")
		...
		const cbpfsPanelClip = cbpfsPanel.main
			.append("clipPath")
		...
		const yScaleDonors = d3.scalePoint().padding(0.5);

		const yScaleCbpfs = d3.scalePoint().padding(0.5);

		const xAxisDonors = d3
			.axisTop(xScaleDonors)
			.tickSizeOuter(0)
			.ticks(3)
		...
		const groupXAxisCbpfs = cbpfsPanel.main
			.append("g")
			.attr("class", "pbiclcgroupXAxisCbpfs")
			.attr("clip-path", "url(#pbiclcCbpfsPanelClip)")
			.attr("transform", "translate(0," + cbpfsPanel.padding[0] + ")");

		const groupYAxisDonors = donorsPanel.main
			.append("g")
			.attr("class", "pbiclcgroupYAxisDonors");

		const groupYAxisCbpfs = cbpfsPanel.main
			.append("g")
		...
		if (isPfbiSite && !isBookmarkPage) {
			...
			Promise.all([
				fetchFile(
					classPrefix + "data_" + selectedDateFromUrl,
					dataUrl,
					"contributions data",
					"csv"
				),
		...
		function csvCallback([rawData, flagsData, masterRegionalFunds]) {
			...
			yearsArray = rawData
				.map(function (d) {
					if (
						d.GMSDonorISO2Code &&
						!countryNames[d.GMSDonorISO2Code.toLowerCase()]
					)
						countryNames[d.GMSDonorISO2Code.toLowerCase()] =
							d.GMSDonorName;
					if (
						d.PooledFundISO2Code &&
						!d.PooledFundName.toLowerCase().includes("closed") &&
						!countryNames[d.PooledFundISO2Code.toLowerCase()]
					)
						countryNames[d.PooledFundISO2Code.toLowerCase()] =
							d.PooledFundName;
			...
			dispatchYearSelectionToPage();

			const allDonors = rawData
				.map(function (d) {
			...
			if (!lazyLoad) {
				...
				d3.select("body").on("d3ChartsYear.pbiclc", function () {
					...
					if (chartState.selectedYear[0] > currentYear)
		...
		function draw([rawData, flagsData, masterRegionalFunds]) {
			...
			function clickButtonsRects(d, singleSelection) {
				...
				d3.selectAll(".pbiclcbuttonsRects").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? unBlue
						: "#eaeaea";
				});

				d3.selectAll(".pbiclcbuttonsText").style("fill", function (e) {
					return chartState.selectedYear.indexOf(e) > -1
						? "white"
						: "#444";
				});
				...
				syncBrowserUrlFromChartQueryString();
				dispatchYearSelectionToPage();
			...
			function clickButtonsContributionsRects(d) {
				...
				d3.selectAll(".pbiclcbuttonsContributionsRects").style(
					"fill",
					function (e) {
			...
			function createSVGLegend() {
				const legend = svg.selectAll(".pbiclcSvgLegend").data([true]);
			...
			function createTopPanel() {
				...
				const topPanelMoneyBag = topPanel.main
					.selectAll(".pbiclctopPanelMoneyBag")
					.data([true])
					.enter()
					.append("g")
					.attr(
				...
				const previousValue =
					d3.select(".pbiclctopPanelMainValue").size() !== 0
						? d3.select(".pbiclctopPanelMainValue").datum()
						: 0;

				const previousDonors =
					d3.select(".pbiclctopPanelDonorsNumber").size() !== 0
						? d3.select(".pbiclctopPanelDonorsNumber").datum()
						: 0;
				...
				let mainValueGroup = topPanel.main
					.selectAll(".pbiclcmainValueGroup")
					.data([true]);
				...
				let topPanelMainValue = mainValueGroup
					.selectAll(".pbiclctopPanelMainValue")
					.data([mainValue]);
				...
				let topPanelMainText = mainValueGroup
					.selectAll(".pbiclctopPanelMainText")
					.data([mainValue]);
				...
				let topPanelSubText = mainValueGroup
					.selectAll(".pbiclctopPanelSubText")
					.data([true]);
				...
				let topPanelDonorsNumber = mainValueGroup
					.selectAll(".pbiclctopPanelDonorsNumber")
				...
				let topPanelDonorsText = mainValueGroup
					.selectAll(".pbiclctopPanelDonorsText")
					.data([true]);

				topPanelDonorsText = topPanelDonorsText
					.enter()
					.append("text")
			...
			function createButtonPanel() {
				...
				const buttonsText = buttonsGroup
					...
					.text(function (d) {
						return d === allYearsOption
							? capitalize(allYearsOption)
							: d;
					});

				const buttonsContributionsGroup = buttonPanel.main
					.append("g")
				...
				const buttonsContributionsRects = buttonsContributionsGroup
					.selectAll(null)
					...
					.attr("x", function (_, i) {
						return (
							i * buttonPanel.buttonContributionsWidth +
							buttonPanel.buttonPadding / 2
						);
					})
					.style("fill", function (d) {
				...
				const buttonsContributionsText = buttonsContributionsGroup
					.selectAll(null)
					.data(contributionType)
					.enter()
					.append("text")
					...
					.attr("x", function (_, i) {
						return (
							i * buttonPanel.buttonContributionsWidth +
							buttonPanel.buttonContributionsWidth / 2
						);
					})
					.style("fill", function (d) {
						return d === chartState.selectedContribution
			...
			function createDonorsPanel() {
				...
				if (chartState.selectedCbpfs.length === 0) {
					...
					const selectedCbpfsData = data.dataCbpfs
						.filter(function (d) {
							return (
								chartState.selectedCbpfs.indexOf(d.isoCode) > -1
							);
						})
						.map(function (d) {
							return d.donors;
				...
				if (!chartState.selectedCbpfs.length) {
					...
					cbpfRecipient = chartState.selectedCbpfs
						.sort(function (a, b) {
				...
				donorsArray.sort(function (a, b) {
					return (
						b[chartState.selectedContribution] -
							a[chartState.selectedContribution] ||
				...
				yScaleDonors.range([
					donorsPanel.padding[0],
					donorsArray.length * lollipopGroupHeight +
						donorsPanel.padding[0],
				]);
				...
				let donorsPanelTitle = donorsPanel.main
					.selectAll(".pbiclcDonorsPanelTitle")
					.data([true]);

				donorsPanelTitle = donorsPanelTitle
					.enter()
					.append("text")
				...
				let donorGroup = donorsPanel.main
					.selectAll(".pbiclcDonorGroup")
					.data(donorsArray, function (d) {
						return d.isoCode;
					});

				const donorGroupExit = donorGroup.exit().remove();

				const donorGroupEnter = donorGroup
					.enter()
					.append("g")
				...
				const donorStickEnter = donorGroupEnter
					.append("rect")
				...
				const donorLollipopEnter = donorGroupEnter
					.append("circle")
				...
				const donorFlagEnter = donorGroupEnter
					.append("image")
					...
					.attr("href", d => {
						...
						if (
							d.isoCode &&
							!flagsData[d.isoCode.toLowerCase()] &&
							!isPfbiSite
						)
				...
				const donorPaidIndicatorEnter = donorGroupEnter
					.append("path")
				...
				const donorTooltipRectangleEnter = donorGroupEnter
					.append("rect")
				...
				donorGroup
					.select(".pbiclcDonorStick")
					.transition()
					.duration(duration)
					.attr("x", donorsPanel.padding[3])
					.attr("width", function (d) {
						return (
							xScaleDonors(d[chartState.selectedContribution]) -
							donorsPanel.padding[3]
						);
					});

				donorGroup
					.select(".pbiclcDonorLollipop")
					.transition()
					.duration(duration)
					.attr("cx", function (d) {
				...
				donorGroup
					.select(".pbiclcDonorLabel")
					.transition()
					.duration(duration)
					.attr("x", function (d) {
						return (
							xScaleDonors(d[chartState.selectedContribution]) +
							donorsPanel.labelPadding
						);
					})
					.tween("text", function (d) {
						const node = this;
					...
					.on("end", function (d) {
						if (
							d[chartState.selectedContribution] > 0 &&
							d[chartState.selectedContribution] < 1000
						)
							d3.select(this).text("<1k");
					});
				...
				function mouseoverTooltipRectangle(datum) {
					...
					tooltip
						...
						.html(
							"Donor: <strong>" +
								datum.donor +
								"</strong><br><br><div style='margin:0px;display:flex;flex-wrap:wrap;width:262px;'><div style='display:flex;flex:0 54%;'>Total contributions:</div><div style='display:flex;flex:0 46%;justify-content:flex-end;'><span class='contributionColorHTMLcolor'>$" +
								formatMoney0Decimals(datum.total) +
								"</span></div><div style='display:flex;flex:0 54%;white-space:pre;'>Total paid <span style='color: #888;'>(" +
								formatPercentCustom(datum.paid, datum.total) +
								")</span>:</div><div style='display:flex;flex:0 46%;justify-content:flex-end;'><span class='contributionColorHTMLcolor'>$" +
								formatMoney0Decimals(datum.paid) +
				...
				function clickTooltipRectangle(datum) {
					...
					if (!datum.clicked) {
						...
						if (
							chartState.selectedDonors.indexOf(datum.isoCode) ===
							-1
						) {
							chartState.selectedDonors.push(datum.isoCode);
						}
					...
					const allCountries = chartState.selectedDonors
						.map(function (d) {
							return allCbpfs.indexOf(d) > -1
								? countryNames[d] + "@donor"
								: countryNames[d];
						})
						.join("|");
			...
			function createCbpfsPanel() {
				...
				if (chartState.selectedDonors.length === 0) {
					...
					const selectedDonorsData = data.dataDonors
						.filter(function (d) {
							return (
								chartState.selectedDonors.indexOf(d.isoCode) >
								-1
							);
						})
						.map(function (d) {
				...
				if (!chartState.selectedDonors.length) {
					...
					donorCountry = chartState.selectedDonors
						.sort(function (a, b) {
							return a.toLowerCase() < b.toLowerCase()
								? -1
								: a.toLowerCase() > b.toLowerCase()
				...
				yScaleCbpfs.range([
					cbpfsPanel.padding[0],
					cbpfsArray.length * lollipopGroupHeight +
						cbpfsPanel.padding[0],
				]);
				...
				let cbpfsPanelTitle = cbpfsPanel.main
					.selectAll(".pbiclcCbpfsPanelTitle")
					.data([true]);

				cbpfsPanelTitle = cbpfsPanelTitle
					.enter()
					.append("text")
				...
				let cbpfGroup = cbpfsPanel.main
					.selectAll(".pbiclcCbpfGroup")
					.data(cbpfsArray, function (d) {
						return d.isoCode;
					});

				const cbpfGroupExit = cbpfGroup.exit().remove();
				...
				const cbpfStickEnter = cbpfGroupEnter
					.append("rect")
				...
				const cbpfLollipopEnter = cbpfGroupEnter
					.append("circle")
				...
				const cbpfPaidIndicatorEnter = cbpfGroupEnter
					.append("path")
				...
				const cbpfTooltipRectangleEnter = cbpfGroupEnter
					.append("rect")
				...
				cbpfGroup
					.select(".pbiclcCbpfStick")
					.transition()
					.duration(duration)
					.attr("x", cbpfsPanel.padding[3])
					.attr("width", function (d) {
						return (
							xScaleCbpfs(d[chartState.selectedContribution]) -
							cbpfsPanel.padding[3]
						);
					});

				cbpfGroup
					.select(".pbiclcCbpfLollipop")
					.transition()
					.duration(duration)
					.attr("cx", function (d) {
				...
				cbpfGroup
					.select(".pbiclcCbpfLabel")
					.transition()
					.duration(duration)
					.attr("x", function (d) {
						return (
							xScaleCbpfs(d[chartState.selectedContribution]) +
							cbpfsPanel.labelPadding
						);
					})
					.tween("text", function (d) {
						const node = this;
				...
				function mouseoverTooltipRectangle(datum) {
					...
					tooltip
						...
						.html(
							"CBPF: <strong>" +
								datum.cbpf +
								"</strong><br><br><div style='margin:0px;display:flex;flex-wrap:wrap;width:290px;'><div style='display:flex;flex:0 62%;'>Total contributions:</div><div style='display:flex;flex:0 38%;justify-content:flex-end;'><span class='contributionColorHTMLcolor'>$" +
								formatMoney0Decimals(datum.total) +
								"</span></div><div style='display:flex;flex:0 62%;white-space:pre;'>Contribution paid <span style='color: #888;'>(" +
								formatPercentCustom(datum.paid, datum.total) +
								")</span>:</div><div style='display:flex;flex:0 38%;justify-content:flex-end;'><span class='contributionColorHTMLcolor'>$" +
								formatMoney0Decimals(datum.paid) +
								"</span></div><div style='display:flex;flex:0 62%;white-space:pre;'>Contribution unpaid <span style='color: #888;'>(" +
								formatPercentCustom(datum.pledge, datum.total) +
								")</span>:</div><div style='display:flex;flex:0 38%;justify-content:flex-end;'><span class='contributionColorHTMLcolor'>$" +
								formatMoney0Decimals(datum.pledge) +
								"</span></div></div>"
				...
				function clickTooltipRectangle(datum) {
					...
					if (!datum.clicked) {
						...
						if (
							chartState.selectedCbpfs.indexOf(datum.isoCode) ===
							-1
						) {
							chartState.selectedCbpfs.push(datum.isoCode);
						}
					...
					const allCountries = chartState.selectedCbpfs
						.map(function (d) {
							return allDonors.indexOf(d) > -1
								? countryNames[d] + "@fund"
								: countryNames[d];
				...
				function setGroupOpacity() {
					if (!chartState.selectedCbpfs.length) {
						...
						cbpfGroup.style("opacity", function (d) {
							return chartState.selectedCbpfs.indexOf(d.isoCode) >
								-1
								? 1
								: fadeOpacity;
						});
		...
		function resizeSVGHeight(donorsLength, cbpfsLength) {
			donorsPanel.height =
				donorsLength * lollipopGroupHeight +
				donorsPanel.padding[0] +
				donorsPanel.padding[2];

			cbpfsPanel.height =
				cbpfsLength * lollipopGroupHeight +
				cbpfsPanel.padding[0] +
				cbpfsPanel.padding[2];
			...
			height =
				...
				topPanel.height +
				buttonPanel.height +
				Math.max(donorsPanel.height, cbpfsPanel.height) +
				2 * panelHorizontalPadding;
		...
		function setRanges(labelSizeDonors, labelSizeCbpfs) {
			...
			xScaleDonors.range([
				donorsPanel.padding[3],
				donorsPanel.width - donorsPanel.padding[1],
			]);

			xScaleCbpfs.range([
				cbpfsPanel.padding[3],
				cbpfsPanel.width - cbpfsPanel.padding[1],
			]);

			yScaleDonors.range([
				donorsPanel.padding[0],
				donorsPanel.height - donorsPanel.padding[2],
			]);

			yScaleCbpfs.range([
				cbpfsPanel.padding[0],
				cbpfsPanel.height - cbpfsPanel.padding[2],
			]);
		}
		...
		function populateSelectedDescriptionDiv() {
			...
			const selection = chartState.selectedDonors.length
				? "selectedDonors"
				: "selectedCbpfs";
			...
			selectionDescriptionDiv.html(function () {
				...
				const countryList = chartState[selection]
					.map(function (d) {
					...
					.reduce(function (acc, curr, index) {
						return (
							acc +
							(index >= chartState[selection].length - 2
								? index > chartState[selection].length - 2
									? curr
									: curr + " and "
								: curr + ", ")
		...
		function createSnapshot(type, fromContextMenu) {
			...
			html2canvas(imageDiv).then(function (canvas) {
				...
				if (fromContextMenu && currentHoveredRect)
					d3.select(currentHoveredRect).dispatch("mouseout");
			});
		...
		function downloadSnapshotPng(source) {
			...
			const fileName =
				"CBPFcontributions_" + csvDateFormat(currentDate) + ".png";
		...
		function downloadSnapshotPdf(source) {
			...
			d3.image(
				...
				const intro = pdf.splitTextToSize(
					"Since the first CBPF was opened in Angola in 1997, donors have contributed more than $5 billion to 27 funds operating in the most severe and complex emergencies around the world.",
				...
				pdf.fromHTML(
					"<div style='margin-bottom: 2px; font-family: Arial, sans-serif; color: rgb(60, 60 60);'>Date: <span style='color: rgb(65, 143, 222); font-weight: 700;'>" +
						...
						yearsList +
						"</span></div><div style='margin-bottom: 2px; font-family: Arial, sans-serif; color: rgb(60, 60 60);'>Contributions: <span style='color: rgb(65, 143, 222); font-weight: 700;'>" +
						contributions +
						"</span></div><div style='margin-bottom: 2px; font-family: Arial, sans-serif; color: rgb(60, 60 60);'>" +
				...
				function countriesList() {
					const selection = chartState.selectedDonors.length
						? "selectedDonors"
						: "selectedCbpfs";
					const type = chartState.selectedDonors.length
						? "donor"
						: "CBPF";
					const plural =
						chartState[selection].length === 1 ? "" : "s";
					const countryList = chartState[selection]
						.map(function (d) {
							return countryNames[d];
						})
		...
		function validateCountries(countriesString, allDonors, allCbpfs) {
			if (!countriesString || countriesString.toLowerCase() === "none")
				return;
			...
			namesArray.forEach(function (d) {
				...
				if (nameSplit.length === 1) {
					const foundDonor = countryCodes.find(function (e) {
						return (
							countryNames[e].toLowerCase() === nameSplit[0] &&
							allDonors.indexOf(e) > -1
						);
					});
					const foundCbpf = countryCodes.find(function (e) {
						return (
							countryNames[e].toLowerCase() === nameSplit[0] &&
							allCbpfs.indexOf(e) > -1
						);
					});
					...
					if (nameSplit[1] === "donor") {
						const foundDonor = countryCodes.find(function (e) {
							return (
								countryNames[e].toLowerCase() ===
									nameSplit[0] && allDonors.indexOf(e) > -1
							);
						});
						if (foundDonor)
							chartState.selectedDonors.push(foundDonor);
					} else if (nameSplit[1] === "fund") {
						const foundCbpf = countryCodes.find(function (e) {
							return (
								countryNames[e].toLowerCase() ===
									nameSplit[0] && allCbpfs.indexOf(e) > -1
							);
						});
						if (foundCbpf) chartState.selectedCbpfs.push(foundCbpf);
					}
		...
		function formatPercentCustom(dividend, divisor) {
			...
			return +percentage.split("%")[0] === 0 && dividend / divisor !== 0
				? "<1%"
				: percentage;
		}
